...
# Используем timeout чтобы избежать зависания на интерактивных промптах
# Если есть деструктивные изменения, они требуют ручного подтверждения
timeout 30 npm run db:push 2>&1 || echo -e "${YELLOW}Миграции пропущены (timeout или не требуются)${NC}"
...
echo -e "  ./backup-db.sh          - создать бэкап"
echo -e "  ./restore-db.sh --list  - список бэкапов"
echo -e "  ./restore-db.sh --latest - восстановить"
echo ""
